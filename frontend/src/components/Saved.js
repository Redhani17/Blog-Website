...
export default function Saved() {
    ...
    const viewSavedblogs=async()=>{
    const res=await fetch(`http://localhost:3000/view-saved/${id}`,{
        method:"GET",
        headers:{"content-Type":"application/json"},
        credentials:"include"
    });
    const data=await res.json();
    setSavedblogs(data.blog);
    }
  ...
  return (
    ...
    {blogs
      .filter(blog => savedblogs.includes(blog._id))
      .map(blog => (
        <div key={blog._id} style={styles.card}>
                  ...
                  <h4 className='like' onClick={async()=>{await like(blog._id); viewblogs();}}>{blog.likes.includes(userId) ? <><GoHeartFill className='likes'/><span className="tooltip">Unlike</span></>: <><GoHeart className='likes'/><span className="tooltip">Like</span></>}&nbsp;&nbsp;{blog.likes.length}</h4>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                  <h4 className='comment' onClick={async()=>{ setActiveBlog(blog); setShowComments(true); await viewComments(blog._id); viewblogs();}}><FaRegComment className='comments'/>&nbsp;&nbsp;{blog.comments.length}<span className="tooltip">Comments</span></h4>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                  ...
                  {userId===blog.logId &&(
                  <h4 className='delete' onClick={async()=>{await deleteblog(blog._id); viewblogs();}}><ImBin className='deletes'/><span className="tooltip">Delete</span></h4>
                  )}
                  {showComments && (
                <div className="modal-overlay" onClick={() => setShowComments(false)}>
                   ...
                   <h3 className="comment" onClick={() => setShowComments(false)}><RiCloseCircleFill className='comments'/><span className="tooltip">Close</span></h3>
                    ...
                    {viewcomment.map(com => (
                        ...
                        {userId===com.userId && (
                            <div>
                              <h4 className='delete' onClick={async()=>{ await deletecomment(com._id,com.id); await viewComments(com.id); viewblogs();}}><ImBin className='deletes'/><span className="tooltip">Delete</span></h4>
                            </div>
                        )}
                        </div>
                    ))}
                    <div className='add-comment'>
                      ...
                      <h4 className="comment" onClick={async() =>{await addcomment(activeBlog._id); viewblogs();}}><FaComment className='comments'/><span className="tooltip">Comment</span></h4>
                    </div>
                    </div>
                </div>
                )}
